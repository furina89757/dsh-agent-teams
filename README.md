...
> [!IMPORTANT]
> **Fork notice**: this repository is a **feature fork** of [NanmiCoder/dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams), maintained by **furina89757**.
> Changes: ① the status display migrated to a [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) side card (`agent-teams:activity`; falls back to the original floating panel when better-sidebar is absent); ② Windows transient-lock retry test margin fix (`scripts/verify.mjs`).
> See [Install](#install) below for installing this fork.
...
> [!NOTE]
> This fork requires [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) for the side-card surface; the upstream floating panel remains the fallback when it is absent. Hosts on `0.1.0-rc` / `0.1.1-rc` should stay on AgentTeams `<= 0.1.13`.
...
### Install from this fork repository (git source, auto-update ready)

```sh
dsh plugin --profile web add "github:furina89757/dsh-agent-teams"
```

> dsh-market tracks the fork's git HEAD: after pushing new commits you can update with one click in dsh-market.
