...
> [!IMPORTANT]
> **Fork 说明**：本仓库为 [NanmiCoder/dsh-agent-teams](https://github.com/NanmiCoder/dsh-agent-teams) 的**功能分叉**，由 **furina89757** 维护。
> 变更：① 状态展示迁移为 [DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar) 侧边卡（`agent-teams:activity`，未安装时回退为原有浮动面板）；② Windows 瞬态锁重试测试余量修复（`scripts/verify.mjs`）。
> 安装本 fork 的方式见 [安装](#安装) 一节。
...
> dsh-market 会检测 fork 的 git HEAD：推送新提交后即可在 dsh-market 一键更新。
